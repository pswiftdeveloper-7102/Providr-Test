...
// ────────── Mobile nav (hamburger + sheet, web only) ──────────
// Mounted in the web AppHeader. Shows the same nav items as the
// desktop sidebar inside a left-anchored Sheet. Visible below lg only.
// The PWA at /app has its own bottom nav and doesn't use this.

export function MobileNav({ portal, isManager }: Props) {
  const pathname = usePathname();
  if (portal === "worker") return null;
      ...
      <SheetContent side="left" className="w-72 p-0">
        ...
        <div className="overflow-y-auto px-3 py-4">
          {portal === "provider" ? (
            ...
            <MobileFlatNav pathname={pathname} portal={portal} items={SC_NAV} />
          )}
        </div>
      </SheetContent>
    </Sheet>
...
function MobileModuleNav<K extends string>({
  pathname,
  isManager,
  config,
}: {
  ...
  const activeTab = config.tabs.find((t) => t.key === module);
  const items = (isManager
    ? config.itemsByModule[module]
    : config.itemsByModule[module].filter((item) => !item.managerOnly));
